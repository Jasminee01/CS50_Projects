...
function Paddle:AIupdate(ball)
    if ball.dy <= 0  then 
        self.y = math.max(0, (ball.height/2) - (self.height/2) + ball.y)
    else 
        self.y = math.min(VIRTUAL_HEIGHT - self.height,  (ball.height/2) - (self.height/2) + ball.y)
    end 
