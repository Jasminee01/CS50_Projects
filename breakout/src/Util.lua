...
-- takes the first entry, the last entry and itterates over them. 
...
function GenerateQuadsPaddles(atlas)
    local x = 0
    local y = 64
    -- creates quads of diffferent sizes 

    local counter = 1
    local quads = {}
-- itterates over sprite sheet 4 times 
    for i = 0, 3 do
        -- smallest
...
-- NewCode : generate a new power up 
--[[
    Generate and return a table of quads (sub-images) from a given texture atlas
    Parameters:
    atlas: The texture atlas from which quads will be generated
]]
function GeneratePowerup(atlas)
    local x = 0  
    local y = 200 

    -- - Initialize an empty table to store the quads
    local quads =  {}
    for i = 0, 10 do 
        -- Create a new quad at the current (x, y) position with dimensions 16x16
        -- The dimensions are taken from the texture atlas using atlas:getDimensions()
