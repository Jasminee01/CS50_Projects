...
function LevelMaker.createMap(level)
    ...
    for y = 1, numRows do
        ...
        for x = 1, numCols do
            ...
            -- NEWCODE:
            -- Check if the level is greater than or equal to the minimum level required to lock a feature
            if level >= MIN_LEVEL then
                 -- Generate a random number between 1 and 100
                if math.random(1, 100) <= level then
                    -- If the condition is met, set the 'isLocked' flag of object (brick) to true
                    b.isLocked = true
                end
            end
