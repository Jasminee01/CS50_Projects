...
--[[
    Swaps two tiles on the game board if they can be legally swapped. If swapiftrue is true, 
    it performs an animated swap between the tiles.After the swap, it checks if the swap results 
    in a valid match. if a match  is found, it triggers the matching animation, adds points to 
    the player's score based on the matched tiles, and removes them from the board. If no match 
    is found,it swaps the tiles back to their original positions and plays an error sound.
]]
function PlayState:swapTiles(t1, t2, swapiftrue)
  self.board:swapTile(t1,t2)

  if swapiftrue then
    -- Perform an animated swap between the tiles
    Timer.tween(0.1, {
        [t1] = {x = t2.x, y = t2.y},
        [t2] = {x = t1.x, y = t1.y}
        })
...
function PlayState:calculateMatches(calculatedMatches)
    ...
    -- Check if there are potential matches available on the game board.
    if not self.board:potentialMatches() then
        
      -- If no potential matches are found, set the 'noMatches' flag to true.
      self.noMatches = true
      gSounds['error']:play()

      -- Animate the 'noMatchLabelY' property to display a message in the middle of the screen.
      Timer.tween(0.5, {
      ...
      :finish(function()
        ...
        :finish(function()
        -- While there are still no potential matches, reset the game board
        -- until potential matches become available.
          while not self.board:potentialMatches() do
            self.board = Board(VIRTUAL_WIDTH - 272, 16, self.level)
          end
        -- Reset the 'noMatches' flag and hide the message label.
...
function PlayState:render()
    ...
    if self.noMatches then
      ...
      love.graphics.setFont(gFonts['medium'])
      love.graphics.printf('No more matches! Reloading board',
